...
This machine is an interesting one. Enumerating is key here and if done well, can help you gain root access in under 10 minutes! However, I will take a longer path in this write-up more akin to my original experience in solving this machine.

## Reconnaissance

### Rustscan

My methodology is to run `rustscan` and then pipe out its results to `nmap`

```bash
rustscan -a beep.htb -- -A -sC -sV -oN beep.nmap
```

In under 5 seconds, I get the open ports from `rustscan` and the `nmap` scan has started. 

![](../.gitbook/assets/image%20%281%29.png)

> I prefer `rustscan` as I can now start enumerating the websites and other ports by myself while the `nmap` scan runs in the background.

A lot of services happen to be open. More services = More rabbit holes.
...
|_pop3-capabilities: APOP USER PIPELINING EXPIRE(NEVER) AUTH-RESP-CODE IMPLEMENTATION(Cyrus POP3 server v2) UIDL TOP LOGIN-DELAY(0) RESP-CODES STLS
...
|_imap-capabilities: CATENATE THREAD=ORDEREDSUBJECT IMAP4 STARTTLS UNSELECT MULTIAPPEND X-NETSCAPE MAILBOX-REFERRALS URLAUTHA0001 BINARY SORT=MODSEQ UIDPLUS RENAME IMAP4rev1 LISTEXT ACL IDLE LIST-SUBSCRIBED ANNOTATEMORE THREAD=REFERENCES SORT RIGHTS=kxte ID CHILDREN LITERAL+ NO QUOTA OK CONDSTORE NAMESPACE ATOMIC Completed
...
|_-----END CERTIFICATE-----
|_ssl-date: 2021-08-14T13:42:17+00:00; 0s from scanner time.
